...
export default function GroundArrivalPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: r, error: re } = await supabase
        .from("reviews")
        .select("id,visit_date,match,arrival")
  ...
  const items = reviews.filter((r) => (r.arrival ?? "").trim().length > 0);
      ...
      {loading ? (
        ...
        <>
          <header className="space-y-2">
            ...
            <h1 className="text-3xl font-semibold">Anreise — {ground.name}</h1>
          </header>
